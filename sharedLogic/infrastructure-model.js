class InfrastructureModel {
    ...
    getDijkstraProblemForInfrastructure(requiredBandwidth) {
        ...
        this.connections.forEach(function (connection) {
            const weighting = InfrastructureModel._calculateDijkstraWeighting(connection, requiredBandwidth);
            if (weighting !== -1) {
                // only path that do not exceed bandwidth
                problem[connection_from(connection)][connection_to(connection)] = weighting;
            }
        });
    ...
    ***************************************************************** */
    ...
    addUsedBandwidthToConnectionWithId(connectionId, amount) {
        ...
        if (connection_usedBandwidth(connection) > connection_availableBandwidth(connection)) {
            if (this.connectionsWithMissingBandwidth.includes(connectionId) === false) {
    ...
    removeUsedBandwidthFromConnectionWithId(connectionId, amount) {
        ...
        if (this.connectionsWithMissingBandwidth.includes(connectionId) &&
            connection_usedBandwidth(connection) <= connection_availableBandwidth(connection)) {
    ...
    getConnectionIdBetweenNodes(nodeId1, nodeId2) {
        ...
        this.connections.forEach(function (c) {
            if (String(connection_from(c)) === String(nodeId1) && String(connection_to(c)) === String(nodeId2)) {
